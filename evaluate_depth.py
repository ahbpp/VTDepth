...
from utils_monodepth import readlines
...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        encoder, depth_decoder = config.build_depth([opt.height, opt.width])
    ...
    for i in range(pred_disps.shape[0]):
        ...
        # print("gt_depth", gt_depth.max(), gt_depth.mean())
        # print("pred_depth", pred_depth.max(), pred_depth.mean())
        ...
        # print("um_gt_depth", unmasked_gt_depth.max(), unmasked_gt_depth.mean())
        # print("um_pred_depth", unmasked_pred_depth.max(), unmasked_pred_depth.mean())

        unmasked_pred_depths.append(unmasked_pred_depth)
        unmasked_gt_depths.append(unmasked_gt_depth)

    # masks = np.concatenate(masks)
    # masked_pred_depths = np.concatenate(masked_pred_depths)
    # masked_gt_depths = np.concatenate(masked_gt_depths)
    every_n = 70
    save_numpy(masks[::every_n], results_save_dir, "masks.npy")
    save_numpy(unmasked_pred_depths[::every_n], results_save_dir, "pred_depths.npy")
    save_numpy(unmasked_gt_depths[::every_n], results_save_dir, "gt_depths.npy")
    ...
    # output_path = os.path.join(
    #     opt.load_weights_folder, "mean_errors_{}_split.npy".format(opt.eval_split))
    # print("-> Saving mean errors to ", output_path)
